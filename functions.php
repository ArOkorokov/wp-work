...
function bakery_setup() {
	...
	register_nav_menus(
		array(
			...
			'menu-2' => esc_html__( 'footer', 'bakery' ),
		)
	);
 ...
 */
function bakery_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'bakery_content_width', 640 );
}
add_action( 'after_setup_theme', 'bakery_content_width', 0 );
...
function bakery_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'bakery' ),
...
function bakery_scripts() {
	...
	wp_enqueue_script( 'bakery-mainJs', get_template_directory_uri() . '/assets/js/app.js', array(), 1.0, true );
...
require get_template_directory() . '/inc/footerWalker.php';
